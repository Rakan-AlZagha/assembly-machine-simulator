...
void main(void) {
  ...
  while(isValidState) {//while loop based on the state, remains true until user calls quit command
    scanf("%s", command); //scan for initial command
...
Check validMemoryAndRegister(char* memoryLocation, char* destination, const char* possibilities[]) {
  ...
  for(i = 0; i < ARRAY_SIZE; i++) { //loop through array possibilities
   ...
   if(result.location == -1 || result.destination == -1){
...
int locationErrorChecking(Check result) {
  ...
  else if(result.validInput == 0) { //if we get invalid input (LOGIC OFF)
    printf("???\n"); //error message
    return 0; //return false
  }
  else {
